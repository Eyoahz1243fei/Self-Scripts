...
class DetectionModel(BaseModel):
    ...
    def _forward_augment(self, x):
        ...
        s = [1.40, 1.00, 0.70]  # scales
        f = [None, None, None]  # flips (2-ud, 3-lr)
        y = []  # outputs
        ...
        for p_ in y:
            p_ = torch.cat(p_, -1)
            result.append(p_)
        return result, None  # augmented inference, train
    ...
    def _descale_pred(self, p, flips, scale, img_size):
        ...
        for p_ in p:
            if self.inplace:
                p_[..., :4] /= scale  # de-scale
                if flips == 2:
                    p_[..., 1] = img_size[0] - p_[..., 1]  # de-flip ud
                    print("flipup finished")
                elif flips == 3:
                    p_[..., 0] = img_size[1] - p_[..., 0]  # de-flip lr
                    print("fliplr finished")
            else:
                x, y, wh = p_[..., 0:1] / scale, p_[..., 1:2] / scale, p_[..., 2:4] / scale  # de-scale
                if flips == 2:
                    y = img_size[0] - y  # de-flip ud
                elif flips == 3:
                    x = img_size[1] - x  # de-flip lr
                p_ = torch.cat((x, y, wh, p_[..., 4:]), -1)
        return p
    ...
    def _clip_augmented(self, y):
        ...
        for p_ in y[0]:
            i = (p_.shape[-1] // g) * sum(4**x for x in range(e))  # indices
            p_ = p_[..., :-i]  # large
        for p_ in y[-1]:
            i = (p_.shape[-1] // g) * sum(4 ** (nl - 1 - x) for x in range(e))  # indices
            p_ = p_[..., i:]  # small
